...
Cinemática é o ramo da Mecânica que tem como objetivo descrever o movimento dos objetos. Por enquanto, nosso objetivo é descrever o movimento dos objetos matematicamente. Quando já estivermos conseguindo descrever o movimento, vamos passar para o próximo passo que é compreender as causas do movimento.

## 1.1. POSIÇÃO

**Posição** é uma grandeza física que indica a localização de um objeto em relação a um ponto de referência definido. A posição é, portanto, uma grandeza que depende do ponto de referência. Quando se muda o ponto de referência, a posição do objeto também muda.
...
Para diferenciar pontos que estão à direita ou à esquerda do ponto de referência utilizaremos a seguinte convenção:
...
md"""
Na figura acima, o ponto A está a $(abs(xs[1])) m $(sentido(xs[1])) do ponto de referência. Sendo assim iremos representar sua posição como: A = $(xs[1]) m. Já o ponto C está a $(abs(xs[3])) m $(sentido(xs[3])) do ponto de referência, portanto sua posição será C = $(xs[3]) m.
...
Na situação acima, onde podemos considerar apenas 1 dimensão, a distância do objeto até o ponto de referência é simplesmente o valor absoluto de sua posição. 

>A distância é uma grandeza que é sempre positiva!

No exemplo acima, a distância do ponto A até o ponto de referência é $(xs[1]) m. Já a distância do ponto C até o ponto de referência é $(abs(xs[3])) m.
...
Imagine um ponto ``P_1`` cuja posição em relação a um referencial ``O`` é dada por ``x_1``. Agora, imagine um ponto ``P_2`` que tem posição em relação ao mesmo referencial dada por ``x_2``. Se quisermos mudar o ponto de referencia de ``O`` para ``P_2``, o que aconteceria com a posição de ``P_1``?

O ponto ``P``, certamente, não mudou de lugar, mas como houve a mudança do ponto de referência, a medida da posição de ``P_1`` também muda. Encontrar a nova posição de ``P_1`` em relação a ``P_2``, representada por ``P_{1,2}`` é bem simples. Basta apenas subtrair a posição de ``P_1`` pela posição de ``P_2``:
...
md"""
### 1.1.2. Posição sobre uma superfície (2 dimensões)

Já quando precisamos localizar um objeto que está sobre uma superfície, a situação muda e não conseguimos mais representar a posição utilizando apenas um valor. Uma superfície é um ente geométrico que possui duas dimensões, e por isso, precisamoa de fornecer pelo menos dois valores para localizar um ponto. No caso de uma superfície plana como uma mesa, podemos sobrepor um plano cartesiano à superfície, e utilizá-lo para marcar as posições dos objetos ou locais de interesse. 
...
begin
	...
	if mostrar_pos_01
		text!([string("A = (", ponto_A[1], " cm, ", ponto_A[2], " cm)"), 
			...
			position = [(ponto_A[1], ponto_A[2] + 0.3), 
				(ponto_B[1], ponto_B[2] + 0.3), (ponto_C[1], ponto_C[2] + 0.3)],
...
Para medir a distância (``d_1``) entre um ponto ``P_1 = (x_1, \, y_1)`` e o ponto de referência ``O``, iremos utilizar o *Teorema de Pitágoras*. Note que ao representar graficamente o ponto P, como os eixos ``x`` e ``y`` são perpendiculares entre si, as
...
> Lembrando que a distância entre dois pontos é sempre positiva, portanto a distância é sempre a raiz quadrada positiva!
"""
...
Quando estamos sobre uma superfície, e queremos mudar o ponto de referência, devemos seguir o mesmo processo que no caso unidimensional. Seja o ponto ``P_1 = (x_1, \,y_1)`` definido em relação ao ponto ``O``, e um ponto ``P_2 = (x_2, \,y_2)`` também definido em relação ao ponto ``O``. A posição de ``P_1`` em relação ao ponto ``P_2`` será:

```math
P_{1,2} = (x_1 - x_2, \, y_1 - y_2)
```
...
```math
d_{A,B}^2 = (x_A - x_B)^2 + (y_A - y_B)^2
```
"""

# ╔═╡ 6cf04553-4529-4581-9163-efc03465d885
latexstring("d_{A,B}^2 = [", AC[1], "\\,{\\rm cm} - (", BC[1], 
...
latexstring("d_{A,B}^2 = ",(AC[1] - BC[1])^2 + (AC[2] - BC[2])^2,
...
Quando se quer indicar a posição de um objeto no espaço tridimensional são necessários 3 valores distintos. Na representação cartesiana para a posição, temos uma posição no eixo ``x`` e no eixo ``y``, como no caso bidimensional, e temos a posição em um novo eixo ``z``. A representação da posição de um ponto ``P_1`` no espaço cartesiano é:
...
begin
	...
	if mostrar_pos3d
		...
		text!(ax_10, 
			...
			align = (:left, :center, :center), color = :dodgerblue, textsize = 20)
	else
		text!(ax_10, ["A", "B", "C"], 
			position = [(ponto_3A[1], ponto_3A[2], ponto_3A[3] - 1),
...
latexstring("d_A^2 = ",ponto_3A[1]^2 + ponto_3A[2]^2 + ponto_3A[3]^2, "{\\rm \\,
	...
	"{\\rm \\,	cm^2}} = ", round(norm(ponto_3A), digits = 2),"{\\rm \\, cm}")

# ╔═╡ 45c1eef9-f5e3-421b-97fd-074e40726826
md"""
#### Posição relativa entre dois pontos (3D)

A posição relativa entre dois pontos é calculada de forma semelhante aos casos anteriores. Sendo os pontos ``P_1 = (x_1, \,y_1,\, z_1)`` e ``P_2 = (x_2,\, y_2,\, z_2)`` definidos em relação ao ponto de referência ``O``. A posição ``P_{1,2}`` de ``P_1`` em relação a ``P_2`` será: 

```math
P_{1,2} = (x_1 - x_2,\, y_1 - y_2,\, z_1 - z_2)
```

Já a distância entre ``P_1`` e ``P_2`` pode ser calculada aplicando o *Teorema de Pitágoras*:
```math
d_{1,2}^2 = (x_1 - x_2)^2 + (y_1 - y_2)^2 + (z_1 - z_2)^2
```
```math
...
latexstring("A_B = (", ponto_3A[1], "{\\rm \\, cm} -(", ponto_3B[1], 
...
begin
	...
	latexstring("d_{A,B}^2 = ( ", ponto_3AB[1], "\\,{\\rm cm})^2  + (", ponto_3AB[2], 
...
latexstring("d_{A,B}^2 = ",ponto_3AB[1]^2 + ponto_3AB[2]^2 + ponto_3AB[3]^2, 
	"{\\rm \\, cm^2} \\Rightarrow d_{A,B} = \\sqrt{", 
	ponto_3AB[1]^2 + ponto_3AB[2]^2 + ponto_3AB[3]^2, "{\\rm \\,	cm^2}} = ",
	round(norm(ponto_3AB), digits = 2),"{\\rm \\, cm}")
...
- 1 dimensão (1 medida), ``O = 0``:

```math
P_1 = x_1 
```

- 2 dimensões (2 medidas), ``O = (0, 0)``:

```math
P_1 = (x_1, \, y_1)
```

- 3 dimensões (3 medidas), ``O = (0, 0, 0)``:
...
- 1 dimensão, ``P_1 = x_1`` e ``P_2 = x_2``:

```math
P_{1,2} = x_1 - x_2
```

- 2 dimensões, ``P_1 = (x_1,\, y_1)`` e ``P_2 = (x_2,\, y_2)``:

```math
P_{1,2} = (x_1 - x_2, \, y_1 - y_2)
```

- 3 dimensões, ``P_1 = (x_1,\, y_1, \, z_1)`` e ``P_2 = (x_2,\, y_2, \, z_2)``:
